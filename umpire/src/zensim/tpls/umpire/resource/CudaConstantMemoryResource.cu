...
#include "zensim/tpls/umpire/resource/CudaConstantMemoryResource.hpp"
...
CudaConstantMemoryResource::CudaConstantMemoryResource(
...
void* CudaConstantMemoryResource::allocate(std::size_t bytes)
{
  ...
  if (!m_initialized) {
    cudaError_t error = ::cudaGetSymbolAddress(
        (void**)&m_ptr, s_umpire_internal_device_constant_memory);

    if (error != cudaSuccess) {
      UMPIRE_ERROR("cudaGetSymbolAddress failed with error: "
